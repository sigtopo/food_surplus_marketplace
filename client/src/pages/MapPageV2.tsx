    ...
    if (stores && stores.length > 0) {
      ...
      stores.forEach((store) => {
        ...
        marker.addListener("click", () => {
          ...
          // Create info window with store info
          const content = document.createElement("div");
          content.className = "p-3 max-w-xs";
          content.innerHTML = `
            <div>
              <h3 class="font-bold text-base mb-1">${store.name}</h3>
              <p class="text-xs text-gray-600 mb-2">${store.category}</p>
              <button class="text-xs text-orange-600 font-semibold hover:underline">
                    ...
                    {selectedStore.address && (
                      <div className="flex gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-gray-900 text-sm">{selectedStore.address}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {selectedStore.phone && (
                      <div className="flex gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-gray-900 text-sm">{selectedStore.phone}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {selectedStore.email && (
                      <div className="flex gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-gray-900 text-sm break-all">{selectedStore.email}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {storeMeals && storeMeals.length > 0 && (
                      <div className="space-y-2">
                        <p className="font-semibold text-gray-900 text-sm">
                          {language === "en" ? "Latest Meals" : "أحدث الوجبات"}
                        </p>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">
                    {language === "en" ? "All Stores" : "جميع المتاجر"} ({filteredStores?.length || 0})
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 max-h-96 overflow-y-auto">
                  {isLoading ? (
                    <p className="text-gray-600 text-sm">{t("msg.loading")}</p>
